# Curated Events

A collection of seven `.ics` calendar files covering religious, federal, cultural, civic, and honorary observances from 2026 through 2036, suitable for import into any calendar application that supports the iCalendar format.

## Calendars

### christian.ics

Key Protestant liturgical and observance dates. The following events are included for each year in the range:

| Event                       | Description                                                                                      |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| **Ascension Day**           | Commemorates Jesus' ascension into heaven 40 days after his resurrection                         |
| **First Day of Lent**       | Begins the 40-day season of fasting, prayer, and repentance leading to Resurrection Sunday       |
| **First Sunday of Advent**  | Marks the beginning of the four-week season of preparation leading up to Christmas               |
| **Good Friday**             | Commemorates the crucifixion and death of Jesus Christ at Calvary                                |
| **National Day of Prayer**  | First Thursday of May — calls people of faith to unite in prayer for the nation                  |
| **Palm Sunday**             | Commemorates Jesus' triumphal entry into Jerusalem; one week before Resurrection Sunday          |
| **Pastor Appreciation Day** | Second Sunday of October — honors pastors and spiritual leaders                                  |
| **Pentecost**               | Commemorates the descent of the Holy Spirit upon the apostles, 50 days after Resurrection Sunday |
| **Reformation Day**         | October 31 — commemorates Martin Luther's posting of his 95 Theses in 1517                       |
| **Resurrection Sunday**     | The central Christian celebration of Jesus Christ's bodily resurrection                          |

10 events × 11 years = **110 total events**

---

### civic-engagement.ics

Recurring U.S. civic dates plus periodic milestone events. The following events are included:

| Event                               | Description                                                                                   |
| ----------------------------------- | --------------------------------------------------------------------------------------------- |
| **Census Day** _(2030)_             | April 1, 2030 — the official reference date for the U.S. decennial census                     |
| **Daylight Saving Time Begins**     | Second Sunday of March — clocks spring forward one hour at 2:00 AM                            |
| **Daylight Saving Time Ends**       | First Sunday of November — clocks fall back one hour at 2:00 AM                               |
| **Election Day**                    | Tuesday after the first Monday in November; federal, state, and local elections               |
| **Inauguration Day** _(2029, 2033)_ | January 20 following a presidential election year                                             |
| **Tax Day**                         | Deadline for U.S. federal income tax returns; April 15 (or next business day if on a weekend) |

4 regular events × 11 years + Inauguration Day (×2) + Census Day (×1) = **47 total events**

---

### cultural.ics

Widely observed cultural, folk, and secular days. The following events are included for each year in the range:

| Event                        | Description                                                                                            |
| ---------------------------- | ------------------------------------------------------------------------------------------------------ |
| **April Fool's Day**         | April 1 — a lighthearted day of pranks and jokes                                                       |
| **Arbor Day**                | Last Friday of April — encourages the planting and care of trees                                       |
| **Black Friday**             | Day after Thanksgiving — unofficial start of the holiday shopping season                               |
| **Carnival**                 | January 6 — pre-Lent festival known for costumes and parades in Rio, Venice, Trinidad, and New Orleans |
| **Chinese New Year**         | First day of the Chinese lunisolar calendar; begins the 15-day Lunar New Year celebration              |
| **Cinco de Mayo**            | May 5 — commemorates the Mexican Army's victory at the Battle of Puebla (1862)                         |
| **Earth Day**                | April 22 — international event dedicated to environmental awareness                                    |
| **Father's Day**             | Third Sunday of June — honors fathers and paternal figures                                             |
| **Groundhog Day**            | February 2 — American folk tradition centered on Punxsutawney Phil's weather prediction                |
| **Halloween**                | October 31 — an evening of costumes, trick-or-treating, and autumn festivities                         |
| **Mardi Gras (Fat Tuesday)** | New Orleans celebration falling 47 days before Resurrection Sunday                                     |
| **May Day**                  | May 1 — spring festival and International Workers' Day                                                 |
| **Mother's Day**             | Second Sunday of May — honors mothers and maternal figures                                             |
| **New Year's Eve**           | December 31 — celebrations marking the end of the calendar year                                        |
| **St. Patrick's Day**        | March 17 — cultural celebration of Irish heritage                                                      |
| **Valentine's Day**          | February 14 — cultural holiday honoring love and affection                                             |

16 events × 11 years = **176 total events**

---

### federal.ics

All U.S. federally observed public holidays. The following events are included for each year in the range:

| Event                          | Description                                                                                             |
| ------------------------------ | ------------------------------------------------------------------------------------------------------- |
| **Christmas Day**              | December 25 - celebration of Christ's birth. The secular view focuses on Santa Claus                    |
| **Columbus Day**               | Second Monday of October — celebrates the anniversary of Christopher Columbus' arrival in the Americas  |
| **Independence Day**           | July 4 — celebrates the adoption of the Declaration of Independence (1776)                              |
| **Juneteenth**                 | June 19 — commemorates the emancipation of enslaved African Americans                                   |
| **Labor Day**                  | First Monday of September — honors the American labor movement                                          |
| **Martin Luther King Jr. Day** | Third Monday of January                                                                                 |
| **Memorial Day**               | Last Monday of May — honors those who died in U.S. military service                                     |
| **New Year's Day**             | January 1                                                                                               |
| **Presidents' Day**            | Third Monday of February (officially Washington's Birthday)                                             |
| **Thanksgiving Day**           | Fourth Thursday of November                                                                             |
| **Veterans Day**               | November 11 — honors all U.S. military veterans                                                         |

11 events × 11 years = **121 total events**

---

### honorary.ics

Honorary, military, and national remembrance dates. The following events are included for each year in the range:

| Event                                     | Description                                                                              |
| ----------------------------------------- | ---------------------------------------------------------------------------------------- |
| **Alamo Day**                             | March 6 — commemorates the fall of the Alamo in 1836 and the defenders who died there    |
| **Armed Forces Day**                      | Third Saturday of May — honors all currently serving U.S. military members               |
| **D-Day Remembrance**                     | June 6 — anniversary of the 1944 Normandy landings (Operation Overlord)                  |
| **Flag Day**                              | June 14 — commemorates the adoption of the U.S. flag (1777)                              |
| **National Pearl Harbor Remembrance Day** | December 7 — anniversary of the 1941 attack that brought the U.S. into World War II      |
| **National POW/MIA Recognition Day**      | Third Friday of September — honors American prisoners of war and those missing in action |
| **Patriot Day**                           | September 11 — remembrance of the 2,977 killed in the September 11, 2001 attacks         |
| **U.S. Air Force Birthday**               | September 18 — established as an independent branch on September 18, 1947                |
| **U.S. Army Birthday**                    | June 14 — founded by the Continental Congress on June 14, 1775                           |
| **U.S. Marine Corps Birthday**            | November 10 — established at Tun Tavern, Philadelphia, on November 10, 1775              |
| **U.S. Navy Birthday**                    | October 13 — Continental Navy authorized on October 13, 1775                             |
| **U.S. Space Force Birthday**             | December 20 — established on December 20, 2019                                           |

12 events × 11 years = **132 total events**

---

### jewish.ics

Key Jewish feasts and holy days. The following events are included for each year in the range:

| Event                                    | Description                                                                                           |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **Hanukkah (Feast of Dedication)**       | Eight-day Festival of Lights celebrating the rededication of the Temple (~165 BC); 25 Kislev          |
| **Passover (Feast of Unleavened Bread)** | Commemorates the Exodus from Egypt; begins 15 Nisan with the Passover Seder                           |
| **Purim (Feast of Lots)**                | Celebrates the deliverance of the Jewish people as recorded in the Book of Esther; 14 Adar            |
| **Rosh Hashanah (Feast of Trumpets)**    | The Jewish New Year; begins the Ten Days of Awe leading to Yom Kippur; 1 Tishrei                      |
| **Shavuot (Feast of Weeks)**             | Celebrates the giving of the Torah at Sinai, 50 days after the start of Passover; 6 Sivan             |
| **Sukkot (Feast of Tabernacles)**        | Seven-day harvest festival; Jews dwell in a sukkah commemorating the wilderness wandering; 15 Tishrei |
| **Yom Kippur (Day of Atonement)**        | The holiest day of the Jewish year — a 25-hour fast of prayer and repentance; 10 Tishrei              |

7 events × 11 years = **77 total events**

---

### catholic.ics

Key Catholic feast days and observances. The following events are included for each year in the range:

| Event               | Description                                                                                |
| ------------------- | ------------------------------------------------------------------------------------------ |
| **All Saints' Day** | Holy day of obligation honoring all the saints who have attained heaven; November 1        |
| **All Souls' Day**  | Day of prayer and remembrance for the faithful departed; November 2                        |
| **Ash Wednesday**   | Marks the beginning of Lent; Catholics receive ashes as a sign of mortality and repentance |

3 events × 11 years = **33 total events**

---


## Coverage

All calendars cover: **2026, 2027, 2028, 2029, 2030, 2031, 2032, 2033, 2034, 2035, 2036**

## Usage

### Subscribe via URL

Many calendar apps support subscribing to a calendar via URL, which keeps the calendar in sync automatically:

```
https://chadkuehn.github.io/CuratedEvents/christian.ics
```

```
https://chadkuehn.github.io/CuratedEvents/catholic.ics
```

```
https://chadkuehn.github.io/CuratedEvents/jewish.ics
```

```
https://chadkuehn.github.io/CuratedEvents/civic-engagement.ics
```

```
https://chadkuehn.github.io/CuratedEvents/cultural.ics
```

```
https://chadkuehn.github.io/CuratedEvents/federal.ics
```

```
https://chadkuehn.github.io/CuratedEvents/honorary.ics
```

Use the "Add calendar from URL" or "Subscribe" feature in your app and paste the desired URL.

### Import into a Calendar App

1. Download the desired `.ics` file or use the URL above
2. Open your calendar application (Google Calendar, Apple Calendar, Proton Calendar, Outlook, etc.)
3. Use the import or "Add calendar from file" feature to load the `.ics` file

### Proton Calendar Notes

Proton Calendar has the following limitations to be aware of:

- Maximum file size: 10 MB
- Maximum events per import: 15,000
- Date range support appears to cap around 12–13 years out

All files are sized well within those constraints.

## Format

The files follow the [iCalendar (RFC 5545)](https://datatracker.ietf.org/doc/html/rfc5545) standard:

- `CALSCALE:GREGORIAN`
- All events are all-day (`VALUE=DATE`)
- Timezone reference: `America/Chicago`
- Each event has a unique `UID` for deduplication on re-import
